...
function formatValue(value, fm, cc) {
    ...
    if (isCTF(value)) {
        ...
        return formatValue(resolveFunc(value.toPostgres, value), fm, cc);
    }
...
function isCTF(value) {
    return !isNull(value) && typeof value.toPostgres === 'function';
}
...
function $formatQuery(query, values, raw, options) {
    ...
    if (isCTF(values)) {
        // Custom Type Formatting
        return $formatQuery(query, resolveFunc(values.toPostgres, values), raw || values._rawType, options);
    }

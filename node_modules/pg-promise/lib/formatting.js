...
function formatValue(value, fm, cc) {
    ...
    if (value && typeof value === 'object') {
        const ctf = value.toPostgres; // custom type formatting;
        if (typeof ctf === 'function') {
            fm |= value._rawType ? fmFlags.raw : 0;
...
// 2. If 'values' is an object that supports function toPostgres, either its
...
function $formatQuery(query, values, raw, options) {
    ...
    if (values && typeof values === 'object') {
        const ctf = values.toPostgres; // custom type formatting;
        if (typeof ctf === 'function') {
            return $formatQuery(query, resolveFunc(ctf, values), raw || values._rawType, options);
        }
        if (values instanceof Array) {
...
const $as = {
    ...
    format: (query, values, options) => {
        if (query && typeof query.toPostgres === 'function') {
            query = query.toPostgres.call(query, query);
        }
